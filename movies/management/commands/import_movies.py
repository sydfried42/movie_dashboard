            ...
            for row in reader:
                ...
                try:
                    movie = Movie(
                        ...
                        series_title=row['Series_Title'],  # Make sure this matches the column name in the CSV
                        released_year=int(row['Released_Year']),  # Ensure the year is an integer
                        certificate=row['Certificate'] if row['Certificate'] else None,
                        runtime=row['Runtime'],
                        genre=row['Genre'],
                        imdb_rating=float(row['IMDB_Rating']),  # Convert IMDB rating to float
                        ...
                        gross=float(row['Gross'].replace(',', '')) if row['Gross'] else None,  # Remove commas from Gross and convert to float
                    )
                    movie.save()
